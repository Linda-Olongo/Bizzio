...
def init_routes(flask_app, database, models, mail_instance):
    ...
    # Envoi d'une notification admin (pour que les admins voient toutes les actions)
    def notify_admin(*, action: str, resource_type: str, resource_id: Optional[str],
                    message: str, ville: Optional[str] = None):
        """
        Fonction de notification pour le panel admin.
        scope = 'admin' pour toutes les notifications admin (visibles par tous les admins)
        ville est utilisée pour identifier la source de l'action
        """
        rtype = (resource_type or '').lower()
        scope = 'admin'  # Scope spécial pour les notifications admin
        _ville = ville  # Garde la ville pour contexte
    ...
    def api_create_proforma():
        ...
        try:
            ...
            # Notification admin pour création de proforma
            try:
                notify_admin(
                    action='create',
                    resource_type='proforma',
                    resource_id=str(proforma_id),
                    message=f"Nouvelle proforma créée: PRO{proforma_id:05d} - Client: {client_name} ({session.get('ville', 'Ville inconnue')}) - Total: {totals.get('total_ttc', 0):,.0f} FCFA",
                    ville=session.get('ville')
                )
            except Exception as _e:
                print(f"[ADMIN NOTIF CREATE PROFORMA WARN] {_e}")
    ...
    def api_update_proforma(proforma_id):
        ...
        try:
            ...
            # Notification admin pour modification de proforma
            try:
                notify_admin(
                    action='update',
                    resource_type='proforma',
                    resource_id=str(proforma_id),
                    message=f"Proforma modifiée: PRO{proforma_id:05d} - Client: {client_data.get('nom', 'Inconnu')} ({ville}) - Total: {total_ttc:,.0f} FCFA",
                    ville=ville
                )
            except Exception as _e:
                print(f"[ADMIN NOTIF UPDATE PROFORMA WARN] {_e}")
    ...
    def api_update_proforma_status(proforma_id):
        ...
        try:
            ...
            try:
                ...
                # Notification admin pour changement de statut de proforma
                try:
                    # Récupérer le nom du client pour la notification
                    cur.execute("""
                        SELECT c.nom FROM proformas p 
                        LEFT JOIN clients c ON p.client_id = c.client_id 
                        WHERE p.proforma_id = %s
                    """, [proforma_id])
                    client_result = cur.fetchone()
                    client_nom = client_result[0] if client_result else "Client inconnu"
                    
                    notify_admin(
                        action=f'proforma_status_{nouveau_statut}',
                        resource_type='proforma',
                        resource_id=str(proforma_id),
                        message=f"Statut proforma changé: PRO{proforma_id:05d} - Client: {client_nom} ({ville}) - Nouveau statut: {nouveau_statut}",
                        ville=ville
                    )
                except Exception as _e:
                    print(f"[ADMIN NOTIF STATUS PROFORMA WARN] {_e}")
